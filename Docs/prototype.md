...
2. Поддерживается 2 варианта выдачи: `JSON` (по-умолчанию) и `Protobuf`. Для использования последнего необходимо добавить хэдер `Accept: application/protobuf`.
3. В выдаче `JSON` отсутствуют поля, если их значение совпадает с дефолтным (0 для int, пустая строка для строк etc). Это [ограничение `Protobuf`](https://developers.google.com/protocol-buffers/docs/proto3#default).
...
Возможные ошибки
```
Пользователь уже залогинен (есть непустой хэдер X-Session)
{
    error_code: 405,
...
limit - количество тем на одной странице (необязательный, от 5 до 50 включительно, по-умолчанию 20)
```
Ответ
```
{
...
id - id темы
page - номер страницы (необязательный, по-умолчанию 1)
limit - лимит сообщений в теме (необязательный, от 5 до 50 включительно, по-умолчанию 20)
```
Ответ
```
{
    ...
    forum: {                                # данные о форуме, в который входит тема
        id: Int,                            # id форума
        title: String                       # название форума
    },
    messages:[                              # список сообщений в теме
        {
            ...
            text: String,                   # текст сообщения
            is_censored: Boolean,           # текст сообщения изъят модератором?
            stats: {                        # статистика сообщения
                rating: Int                 # рейтинг сообщения
            }
        },
        ...
    ]
...
Запрос
```
GET /v1/blogs?page={page}&limit={limit}&sort={sort}
```
Параметры
```
page - номер страницы (необязательный, по-умолчанию 1)
limit - количество авторских колонок на одной странице (необязательный, от 5 до 50 включительно, по-умолчанию 50)
sort - вариант сортировки (update - по дате выхода последней статьи / article - по количеству статей / subscriber - по количеству подписчиков; необязательный, по-умолчанию update)
```
Ответ
```
{
...
page - номер страницы (необязательный, по-умолчанию 1)
limit - количество статей на одной странице (необязательный, от 5 до 50 включительно, по-умолчанию 5)
```
Ответ
```
{
