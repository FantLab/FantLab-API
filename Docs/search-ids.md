...
Запрос на множественную выдачу позиций базы по id.  
Структура ответа подобна ответам на запросы этих позиций отдельным запросом, исключая некоторые поля.
...
```
a - список id-авторов
w - список id-произдвений
e - список id-издания
```
id передаются через запятую. Не более 200 позиций.

Пример
> [/search-ids.json?a=1,2&w=1,2&e=1,2](https://api.fantlab.ru/search-ids.json?a=1,2&w=1,11&e=3,31,1033) - Выдача данных карточек авторов с id 1 и 2, произведений 1 и 11, издания 3, 31, 1033 (3 не выдастся, т.к. издания с таким ID нет в базе)
...
```
{
  authors: [
    {
        type: "autor",           # сущность/раздел базы (autor, work, edition)
  ...
  works: [
    {
        type: "work",            # сущность/раздел базы (autor, work, edition)
  ...
  editions: [
    {
        type: "edition",         # сущность/раздел базы (autor, work, edition)
        id: Int,                 # id издания
        ...
        creators: {              # массив авторов/составителей/издательств - т.е. тех, кто принял участие в выходе издания
            ...
            publishers: [           # издательства
                {
                    type: "publisher",    # сущность. тут только тип "publisher"
