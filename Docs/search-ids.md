...
Основные поля миникарточки (универсальны для всех сигментов):
```
        type: String,            # сущность/раздел базы: work, edition, author/autor, art и пр. 
                                 # полный список сущностей см. в общем README)
        id: Int,                 # id позиции данного сегмента (вместе с type дают fantlab-id: {type}{id}
