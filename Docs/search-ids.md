## Получение нескольких миникарточек по поиску

Основная json-выдача поиска по базе. Разбита по сегменты базы: авторы/произведения/издания ревариантно найденнему по запросу.  
