...
Пример
```
GET /autors/192 - список всех открытых авторов на букву "A"
```
    ...
    list:[
        {
            ...
            fantastic: Int,        # уровень "фантастичности" (0 - не фантаст, 1 - есть фантастические произведения, 2 - фантаст)
...
*Есть поддержка запроса с получением частичных данных - с указанием параметров и сортировки:
> /autor/1?biography=1&awards=1&&biblio_blocks=1&sort=rating*
...
```
{
        ...
        nom: [
            {
                ...
                work_id: Int|null,              # id награжденного произведения (null, если награда относится к автору)
                work_name: String|null,         # название награжденного произведения (null, если награда относится к автору)
                work_rusname: String|null,      # русскоязычное название награжденного произведения (null, если награда относится к автору)
                work_year: Int|null             # год публикации произведения (null, если награда относится к автору)
            },
            ...
        ],
        win: [
    ...
    classificatory: {              # жарны и классификар произведений автора
       ...                         # формат массива аналогичен выдаче {classificatory} в карточке произведения
    }

    analogs: [                  # подборка похожих писателей (по классификатору)
        {
            autor2_id: Int,           # id автора
            midmark: Float,           # ср. оценка произведений автора (рейтинг автора)
            rusname: String,             # русскоязычное имя автора
        },
        ...
    ],
